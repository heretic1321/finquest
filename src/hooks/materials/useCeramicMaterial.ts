...
export function useCeramicMaterial(props: TUseNonStoneMaterialType) {
  ...
  const [
    ...
    color: props.color ?? '#000000', // Ceramic black
    emissiveColor: props.emissiveColor ?? '#000000', // No emissive color
    emissiveIntensity: props.emissiveIntensity ?? 0, // No emissive intensity
    envMapIntensity: props.envMapIntensity ?? 0, // Default environment map intensity
    flatShading: props.flatShading ?? false,
    fog: props.fog ?? true,
    metalness: props.metalness ?? 0.16,
  ...
  // useControls(
  //   'Ceramic Material ' + props.debugDisplayName,
  //   () => ({
  //     transparent: {
