...
interface RedeemMachineAdaptiveScreenProps {
...
const RedeemMachineAdaptiveScreen: React.FC<RedeemMachineAdaptiveScreenProps> = ({
  ...
  const previousModeRef = useRef(rewardMachineMode)
  ...
  useEffect(() => {
    if (rewardMachineMode !== previousModeRef.current) {
      ...
      } else if (rewardMachineMode === RewardMachineMode.TRAVELLING_INSIDE_TO_OUTSIDE) {
        setTransitionProgress(0)
      } else if (rewardMachineMode === RewardMachineMode.OUTSIDE) {
      ...
      previousModeRef.current = rewardMachineMode
    }
  }, [rewardMachineMode, camera])

  useEffect(() => {
    const handleEscapeKey = (event: KeyboardEvent) => {
      if (event.key === 'Escape' && rewardMachineMode === RewardMachineMode.INSIDE) {
  ...
  useFrame((_, delta) => {
    if (rewardMachineMode === RewardMachineMode.TRAVELLING_OUTSIDE_TO_INSIDE ||
      ...
      const isReturning = rewardMachineMode === RewardMachineMode.TRAVELLING_INSIDE_TO_OUTSIDE
      ...
      if (newProgress === 1) {
        MeteorManagerStore.setState({ rewardMachineMode: isReturning ? RewardMachineMode.OUTSIDE : RewardMachineMode.INSIDE })
      }
    }
  })
...
export default RedeemMachineAdaptiveScreen